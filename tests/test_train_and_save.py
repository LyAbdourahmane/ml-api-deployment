...
def test_train_and_save(tmp_path):
    ...
    # Exemple : 10 lignes, 5 de chaque classe
    df = pd.DataFrame({
        'PrimaryPropertyType': ['Type1']*5 + ['Type2']*5,
    ...
    assert 'feature_names' in loaded_metadata
    assert len(loaded_metadata['feature_names']) > 0
